document.addEventListener("DOMContentLoaded", function () {
    const tableBody = document.getElementById("productionReportTableBody");
    ...
    if (tableBody) {
        tableBody.addEventListener("click", function (e) {
            ...
            if (deleteBtn) {
                ...
                // Try to get production ID from table
                const cells = row.querySelectorAll("td");
                if (cells.length > 1) {
                    productionId = cells[1].textContent.trim();
                }

                const message = productionId
                    ? `Are you sure you want to delete Production Report "${productionId}"?`
    ...
    async function confirmDelete() {
        ...
        try {
            const response = await fetch(`/production/online-production-report/delete/${currentIdToDelete}/`, {
                method: "POST",
                headers: {
            ...
            if (data.success) {
                showSuccessPopup(data.message || "Online Production Report deleted successfully!");
                ...
                alert(data.message || "Failed to delete production report. Please try again.");
            }
        } catch (error) {
            console.error("Error deleting production report:", error);
            alert("Network error while deleting production report. Please try again.");
        }
    }
